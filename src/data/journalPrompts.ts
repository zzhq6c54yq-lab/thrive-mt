...
// Get prompts for a specific category with type safety
export const getPromptsForCategory = (category: string): string[] => {
  if (isValidCategory(category)) {
    return journalPrompts[category];
  }
  return journalPrompts.childhood; // Default to childhood if category not found
};

// Type guard to check if the category is a valid key of journalPrompts
export const isValidCategory = (category: string): category is keyof typeof journalPrompts => {
  return category in journalPrompts;
};
