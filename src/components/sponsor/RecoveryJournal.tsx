...
import { Calendar as CalendarIcon, Save, Pencil, Star, Heart, File, MessageCircle, Cloud, Loader2 } from "lucide-react";
...
const moodToScore: Record<string, number> = { great: 10, good: 8, okay: 5, difficult: 3, struggling: 1 };
const scoreToMood = (score: number | null): JournalEntry["mood"] => {
  if (!score || score <= 2) return "struggling";
...
const RecoveryJournal: React.FC = () => {
  ...
  // Fetch existing journal entries from Supabase
  useEffect(() => {
    const fetchEntries = async () => {
      ...
      if (!error && data) {
        setEntries(data.map(e => ({
          ...
          title: (e.notes || "").split("\n")[0]?.substring(0, 60) || "Journal Entry",
          content: e.notes || "",
          mood: scoreToMood(e.mood_score),
          gratitude: [],
        })));
      }
      setLoading(false);
  ...
  const handleSaveEntry = async () => {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      toast({ title: "Not logged in", description: "Please sign in to save entries.", variant: "destructive" });
      return;
    }

    setSaving(true);
    const gratitudeText = gratitudeItems.filter(g => g.trim()).join("; ");
    const fullNotes = `${title}\n\n${content}${gratitudeText ? `\n\nGratitude: ${gratitudeText}` : ""}`;

    const { data, error } = await supabase.from("journal_entries").insert({
      user_id: user.id,
      mood: mood,
      mood_score: moodToScore[mood],
      notes: fullNotes,
    ...
    toast({ title: "Journal Entry Saved", description: "Your recovery journal entry has been saved." });
      ...
      <Card className="border-2 border-indigo-100 shadow-lg">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-grow">
                    ...
                    <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Give your entry a title..." className="w-full" />
                  </div>
                  <div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <Button variant="ghost" size="sm" onClick={getRandomPrompt} className="text-purple-600 hover:text-purple-800 h-8">
                      <MessageCircle className="h-4 w-4 mr-1" /> Get Prompt
                    </Button>
                  </div>
                  <Textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Write about your recovery journey today..." rows={8} className="w-full" />
                </div>
                  ...
                  <div className="space-y-2">
                    {gratitudeItems.map((item, index) => (
                      <div key={index} className="flex items-center">
                        ...
                        <Input value={item} onChange={e => handleGratitudeChange(index, e.target.value)} placeholder="I'm grateful for..." className="flex-grow" />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <TabsContent value="history" className="p-6">
              <div className="space-y-6">
                <Input placeholder="Search journal entries..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="mb-4" />
                {loading ? (
                  ...
                  <div className="space-y-4">
                    {filteredEntries.map(entry => (
                      <div key={entry.id} className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                        <div className={`px-4 py-3 border-b flex justify-between items-center ${moodColors[entry.mood]}`}>
                          ...
                          <div className="text-xs">{format(new Date(entry.date), "MMM d, yyyy")}</div>
                        </div>
                        <div className="p-4">
                          ...
                          {entry.gratitude.length > 0 && (
                            <div className="mt-4 pt-3 border-t border-gray-100">
                              <p className="text-sm font-medium text-gray-600 mb-2 flex items-center">
                                <Heart className="h-4 w-4 text-pink-500 mr-1" /> Gratitude List
                              </p>
                              <ul className="list-disc list-inside text-sm text-gray-600 pl-2">
                                {entry.gratitude.map((item, i) => <li key={i}>{item}</li>)}
                              </ul>
                            </div>
                          )}
                        </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 mb-4">{searchTerm ? "Try a different search term" : "Start journaling to see your entries here"}</p>
                    {searchTerm ? (
                      <Button variant="outline" onClick={() => setSearchTerm("")}>Clear Search</Button>
                    ) : (
                      <Button onClick={() => setActiveTab("new-entry")} className="bg-purple-700 hover:bg-purple-800">Create Your First Entry</Button>
                    )}
                  </div>
                )}
              </div>
        ...
        {activeTab === "new-entry" && (
          <CardFooter className="bg-gray-50 border-t px-6 py-4">
            <Button onClick={handleSaveEntry} disabled={saving} className="ml-auto bg-purple-700 hover:bg-purple-800">
