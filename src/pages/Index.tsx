...
const Index = () => {
  ...
  const handlePrevious = () => {
    if (screenState === 'mood') {
      ...
      setScreenState('moodResponse'); // Changed from 'register' to 'moodResponse'
  ...
  const handleSkip = () => {
    // Modified skip flow to go to the subscription page even when skipping registration
  ...
  const handleVisionBoardContinue = () => {
    // Require at least 2 selections from each column to continue
  ...
  return (
    <div className="relative">
      ...
      {/* Henry button that appears on all screens except intro */}
      {screenState !== 'intro' && (
        <HenryButton 
      ...
      {screenState === 'moodResponse' && selectedMood === 'overwhelmed' && (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#221F
